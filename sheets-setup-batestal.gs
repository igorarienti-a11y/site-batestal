// ══════════════════════════════════════════════════════════════
//  Batestal — Skyview  |  Google Sheets Setup + CAPI
//  Colar em: https://script.google.com  (vinculado à planilha)
//  Após colar: rodar configurarTudo() UMA VEZ, depois instalar
//  os acionadores onChange e onEdit manualmente.
// ══════════════════════════════════════════════════════════════

const META_PIXEL_ID     = "SEU_PIXEL_ID";
const META_ACCESS_TOKEN = "SEU_TOKEN";
const META_TEST_CODE    = "";  // test_event_code da Meta — só durante debug, depois deixa vazio
...
const LEADS_HEADERS = [
  'Mês', 'Data', 'Nome', 'Email', 'Telefone', 'Interesse', 'Status',
...
// ─── Índices derivados — imunes a reordenação ──────────────────
const COL_MES      = LEADS_HEADERS.indexOf('Mês');
const COL_DATA     = LEADS_HEADERS.indexOf('Data');
const COL_TELEFONE = LEADS_HEADERS.indexOf('Telefone');
const COL_DATAISO  = LEADS_HEADERS.indexOf('Data ISO');
const NUM_COLS     = LEADS_HEADERS.length;
...
function configurarTudo() {
  ...
  // utmStart = coluna 8 (utm_source), techStart = coluna 13 (Event ID)
  configurarAba('Leads', LEADS_HEADERS, { utmStart: 8, techStart: 13 });
}
...
// ══════════════════════════════════════════════════════════════
//  TRIGGER — onChange
//  Novo lead via edge function → move para topo + formata + envia Lead CAPI
// ══════════════════════════════════════════════════════════════
...
function onChange(e) {
  ...
  try {
    ...
    // Guard: só processa se a linha tem email (campo obrigatório do form)
    const emailCol = headers.indexOf('Email');
    if (emailCol === -1 || !rowData[emailCol]) return;
    ...
    const result = sendToMeta(lead, 2, 'Lead', lead['Event ID'] || null, 'website');
...
// ══════════════════════════════════════════════════════════════
//  TRIGGER — onEdit
//  Mudança de Status dispara evento Meta correspondente
// ══════════════════════════════════════════════════════════════
...
function enviarLinhaSelecionadaComoLead() {
  ...
  if (ss.getActiveSheet().getName() !== 'Leads') {
    ui.alert('Selecione uma célula na aba "Leads" antes.');
    return;
  }

  const sheet = ss.getSheetByName('Leads');
  const row   = ss.getActiveSheet().getActiveRange().getRow();
...
function sendToMeta(lead, rowNumber, eventName, eventId, actionSource) {
  try {
    if (META_PIXEL_ID === 'SEU_PIXEL_ID')         return { success: false, error: 'Pixel não configurado' };
    if (META_ACCESS_TOKEN === 'SEU_TOKEN') return { success: false, error: 'Token não configurado' };

    const props    = PropertiesService.getScriptProperties();
    if (!eventId) log('⚠️ Event ID ausente', { linha: rowNumber });
    ...
    if (ok) props.setProperty(cacheKey, new Date().toISOString());
...
function normalizePhone(phone) {
  ...
  if (d.length === 10) { log('⚠️ Telefone 10 dígitos: ' + phone); return ''; }
  return '';
}
...
function formatarLinha(sheet, row) {
  ...
  if (!dateObj) dateObj = parseDataLocal(String(dataISORaw || '').trim());
  if (!dateObj) dateObj = parseDataLocal(String(dataRawVal || '').trim());
  if (!dateObj) return;

  const pad = n => String(n).padStart(2, '0');
  const yyyy = dateObj.getFullYear();
  sheet.getRange(row, COL_DATA + 1)
    .setValue(`${pad(dateObj.getDate())}/${pad(dateObj.getMonth()+1)}/${yyyy} ${pad(dateObj.getHours())}:${pad(dateObj.getMinutes())}`);
...
function moverLeadsParaOTopo() {
  ...
  const getTs = row => {
    const dv = row[COL_DATA]; const iv = row[COL_DATAISO];
    if (dv instanceof Date && !isNaN(dv)) return dv.getTime();
    if (iv instanceof Date && !isNaN(iv)) return iv.getTime();
    const d = parseDataLocal(String(dv||'').trim()) || parseDataLocal(String(iv||'').trim());
...
function testarEnvio() {
  ...
  const rowData    = sheet.getRange(testRow, 1, 1, headers.length).getValues()[0];
  const testEventId = rowData[headers.indexOf('Event ID')] || null;
  ...
  PropertiesService.getScriptProperties().deleteProperty(testCacheKey);

  const result = enviarLeadQualificado(sheet, testRow, headers, testStatus);

  SpreadsheetApp.getUi().alert(
    'Resultado Teste (linha ' + testRow + ' — ' + testStatus + ')',
    'Meta: ' + (result.success ? '✅ OK' : '❌ ' + (result.error || 'HTTP ' + result.statusCode)) + '\n\n' +
    (META_PIXEL_ID === 'SEU_PIXEL_ID' ? '⚠️ Configure META_PIXEL_ID e META_ACCESS_TOKEN no topo do script' : ''),
...
function simularOnChange() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  onChange({ changeType: 'INSERT_ROW', source: ss });
...
function diagnosticar() {
  ...
  SpreadsheetApp.getUi().alert(
    'Diagnóstico',
    'Meta Pixel: '      + (META_PIXEL_ID === 'SEU_PIXEL_ID' ? '❌ não configurado' : META_PIXEL_ID) + '\n\n' +
...
function limparCache() {
  const props = PropertiesService.getScriptProperties();
  props.getKeys().filter(k => k.startsWith('meta_sent_')).forEach(k => props.deleteProperty(k));
